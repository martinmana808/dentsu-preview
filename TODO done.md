...
• Selective Visibility: Instead of showing every property three times, modify the "Ranges" and "Sizes" views to only show active overrides. If a property isn't being overridden at that level, it should not be shown. So at the top of RANGES and SIZES there should be a button that reads 'Add override' and when clicked it should show a dropdown of all the properties that can be overridden at that level.
• Mode Selection: Implement a way for users to choose if their edits apply to the current size only, a range, or globally, rather than having to jump between tabs to perform different types of work. Probably upon creation or edit of the override a button should appear that reads 'Apply to' and when clicked it should show a dropdown of all the other sizes and ranges where that override can be applied.
...
• Interactive Highlighting: When a user hovers over a range, the interface should highlight all the specific sizes that fall within that range: Global, somehow all the sizes should be highlighted. Range, only the sizes within that range should be highlighted. Size, only the current size should be highlighted. 
- Ranges is something similar to a breakpoint in CSS. @media from here to there, or smaller than, or bigger than. 
