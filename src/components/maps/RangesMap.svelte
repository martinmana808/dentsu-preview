<script lang="ts">
  ...
  import { SIZES, MOCK_RANGES } from './constants';
  import { cn } from '@/lib/utils';
  
  let { onNavigate }: { onNavigate?: (tab: 'ranges' | 'sizes') => void } = $props();

  const getRangeStyle = (isStart: boolean, isEnd: boolean) => {
      let rangeStyle: any = {
        position: 'absolute',
        height: '1rem', // h-4
...
<div class="rounded-lg p-6 w-full">
     <BaseMapTable rowHeaderLabel="Ranges">
        {#each MOCK_RANGES as range, rowIndex}
            <tr class="border-gray-100 hover:bg-gray-50/30 transition-colors">
                <td class="propertyCell sticky left-0 z-10 p-3 font-medium text-gray-800 border-r border-gray-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] whitespace-nowrap">
                    {range.name}
                </td>
                         ...
                         <div class="w-full h-full flex items-center justify-center relative">
                            {#if isInRange}
                                <button 
                                    ...
                                    style={styleToString(getRangeStyle(!!isStart, !!isEnd))}
